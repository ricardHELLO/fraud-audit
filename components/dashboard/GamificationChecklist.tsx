...
export interface GamificationChecklistProps {
  ...
  onActionCompleted?: (actionId: string, creditAwarded: boolean) => void
}
...
export function GamificationChecklist({
  ...
  function handleBugReportSuccess(creditAwarded: boolean) {
    if (creditAwarded) {
      onActionCompleted?.('bug_report', true)
    }
  }

  // --- Render action button for each incomplete item ---

  function renderActionButton(action: GamificationAction) {
    if (completedSet.has(action.id)) return null
    ...
    switch (action.id) {
      case 'feedback':
        ...
        return (
          <Link
            href={`/informe/${latestReport.slug}`}
